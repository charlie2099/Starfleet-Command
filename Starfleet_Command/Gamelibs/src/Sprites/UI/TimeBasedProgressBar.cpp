#include <iostream>
#include "Sprites/UI/TimeBasedProgressBar.hpp"

TimeBasedProgressBar::TimeBasedProgressBar()
{
...
void TimeBasedProgressBar::Update(sf::RenderWindow& window, sf::Time time)
{
...
void TimeBasedProgressBar::Render(sf::RenderWindow &window)
{
...
void TimeBasedProgressBar::SetColour(sf::Color colour)
{
    _insideBarSpriteComponent.GetSprite().setColor(colour);
    _text.setFillColor(colour);
}

void TimeBasedProgressBar::SetPosition(sf::Vector2<float> pos)
{
...
void TimeBasedProgressBar::SetTimeToCompleteTask(float timeInSeconds)
{
    _timeToCompleteTask = timeInSeconds;
}

void TimeBasedProgressBar::SetProgressBarText(const std::string& text)
{
    _progressBarText = text;
    _text.setString(_progressBarText);
}

void TimeBasedProgressBar::SetProgressBarWaitingText(const std::string &text)
{
    _waitingForNextTaskText.setString(text);
}

void TimeBasedProgressBar::SetProgressBarStatus(bool status)
{
    _taskIsProgressing = status;
}

void TimeBasedProgressBar::ResetProgressBar()
{
    _taskIsComplete = false;
    _taskIsProgressing = false;
}

void TimeBasedProgressBar::AddBasicObserver(TimeBasedProgressBar::BasicProgressBarEvent observer)
{
    _basicObservers.insert(observer);
}

void TimeBasedProgressBar::InvokeSimpleEvent(EventID eventId)
{
